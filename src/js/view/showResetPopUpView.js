...
function showPopUp(startNewGame) {
    DOMelements.popup.classList.add(DOMclasses.popUpActive);

    document.addEventListener('click', e => {
        if (!e.target.closest('.popup__container') && e.target !== DOMelements.reset) {
...
function showResetPopUpView(startNewGame) {
    DOMelements.reset.addEventListener('click', () => {
        showPopUp(startNewGame);
    });
}

export default showResetPopUpView;